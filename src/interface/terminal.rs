...
fn handle_evaluate(input: Vec<&str>) {
    ...
    let board = match Board::new_from_fen(fen.as_str(), None) {
        Ok(board) => board,
        Err(error) => {
...
fn prepare_board(parameters: &[&str]) -> Result<Board, String> {
    if parameters.is_empty() {
        return Ok(Board::new_initial_position(None));
    }

    match parameters[0] {
        "fen" => {
            let fen = parameters[1..].join(" ");
            Board::new_from_fen(fen.as_str(), None)
        }
        "moves" => Board::new_from_moves(&parameters[1..], None),
