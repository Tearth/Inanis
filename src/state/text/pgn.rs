...
impl PGNLoader {
    ...
    pub fn new(file_iterator: Lines<BufReader<File>>) -> PGNLoader {
        ...
        PGNLoader { file_iterator, zobrist_container, see_container }
    }
        ...
        for line in pgn.lines() {
            if line.is_empty() {
                ...
                let mut board = match fen.clone() {
                    Some(value) => {
                        let fen_result = Board::new_from_fen(&value, Some(self.zobrist_container.clone()), Some(self.see_container.clone()));
                    ...
                    None => Board::new_initial_position(Some(self.zobrist_container.clone()), Some(self.see_container.clone())),
                };
