...
fn evaluate(target_piece: u8, attackers: u8, defenders: u8, evaluation_parameters: &EvaluationParameters) -> i16 {
    if attackers == 0 {
        return 0;
    }
    ...
    evaluate_internal(attacking_piece_index, target_piece_index, attackers, defenders, evaluation_parameters)
}

/// Recursive function called by [evaluate] to help evaluate a static exchange evaluation result.
fn evaluate_internal(attacking_piece: u8, target_piece: u8, attackers: u8, defenders: u8, evaluation_parameters: &EvaluationParameters) -> i16 {
    if attackers == 0 {
        return 0;
    }

    let target_piece_value = get_piece_value(target_piece, evaluation_parameters);
    ...
    let see_result = evaluate_internal(new_attacking_piece, attacking_piece, defenders, new_attackers, evaluation_parameters);
...
fn get_piece_value(piece_index: u8, evaluation_parameters: &EvaluationParameters) -> i16 {
    match piece_index {
