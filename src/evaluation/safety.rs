...
fn evaluate_color(board: &Bitboard, dangered_king_fields: u32) -> i16 {
    ...
    let opening_score = (dangered_king_fields as i16).pow(2) * board.evaluation_parameters.king_attacked_fields_opening;
    let ending_score = (dangered_king_fields as i16).pow(2) * board.evaluation_parameters.king_attacked_fields_ending;
