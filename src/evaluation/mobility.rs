...
fn evaluate_color(board: &Bitboard, color: u8, dangered_king_fields: &mut u32) -> i16 {
    ...
    let knight_mobility_opening_score = knight_mobility * unsafe { board.evaluation_parameters.mobility_opening[KNIGHT as usize] };
    let knight_mobility_ending_score = knight_mobility * unsafe { board.evaluation_parameters.mobility_ending[KNIGHT as usize] };

    let bishop_mobility_opening_score = bishop_mobility * unsafe { board.evaluation_parameters.mobility_opening[BISHOP as usize] };
    let bishop_mobility_ending_score = bishop_mobility * unsafe { board.evaluation_parameters.mobility_ending[BISHOP as usize] };

    let rook_mobility_opening_score = rook_mobility * unsafe { board.evaluation_parameters.mobility_opening[ROOK as usize] };
    let rook_mobility_ending_score = rook_mobility * unsafe { board.evaluation_parameters.mobility_ending[ROOK as usize] };

    let queen_mobility_opening_score = queen_mobility * unsafe { board.evaluation_parameters.mobility_opening[QUEEN as usize] };
    let queen_mobility_ending_score = queen_mobility * unsafe { board.evaluation_parameters.mobility_ending[QUEEN as usize] };
